import { CreatePlaylistRequest, UpdatePlaylistRequest, PopulateFavList } from "#/@types/audio";
...
export const createPlaylist: RequestHandler = async (req: CreatePlaylistRequest, res) => {
    ...
    if(resId){
        ...
        if(!audio){
            return res.status(404).json({error: "Could not found audio with the provided id!"})
        }
    }
    ...
    res.status(201).json({playlist: {id:newPlaylist._id, title: newPlaylist.title, visibility: newPlaylist.visibility}})
...
export const updatePlaylist: RequestHandler = async (
  ...
  const playlist = await Playlist.findOneAndUpdate(
    { _id: id, owner: req.user.id } as any,
    { title, visibility },
    { new: true }
  );
...
export const deletePlaylist: RequestHandler = async (req: any, res: any) => {
