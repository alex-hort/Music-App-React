...
import { PopulatedOwner, PopulateFavList } from "#/@types/audio";
...
export const getLatestUploads: RequestHandler = async (req, res) => { // 👈 no destructuring
  const list = await Audio.find()
    .sort("-createdAt")
    .limit(10)
    .populate<{ owner: PopulatedOwner }>("owner") // 👈 correct populate generic shape

  const audios = list.map((item) => {
    return {                                          // 👈 object {}, not array []
      id: item._id,                                  // 👈 colon, not semicolon
      title: item.title,
      about: item.about,
